...
| `VIMEO_COOKIES_B64` | Base64-encoded Netscape-format vimeo.com cookies from Minh's browser (Edge). Base64 because plain cookie text loses its required TAB separators when pasted into the GitHub secrets UI. Needed because Vimeo Plus has no API file access and yt-dlp requires a logged-in session. Expires after months — when sync fails with a cookies-expired error, re-export (see §6). |
...
**Refresh the VIMEO_COOKIES_B64 secret (when YouTube sync says cookies expired):**
```bash
# Be logged into vimeo.com in Edge first. Extracts, filters to vimeo-only, base64-encodes, copies to clipboard:
yt-dlp --cookies-from-browser edge --cookies /tmp/vc-full.txt --simulate "https://vimeo.com/VIDEO_ID"
{ echo "# Netscape HTTP Cookie File"; grep -E '^\.?([a-z0-9-]+\.)*vimeo\.com\s' /tmp/vc-full.txt; } | base64 | pbcopy
rm /tmp/vc-full.txt
```
Then GitHub → Settings → Secrets → Actions → `VIMEO_COOKIES_B64` → Update → paste. (Base64, not plain text — the GitHub UI mangles the TAB separators otherwise.)
