...
- **🔴 VIMEO COOKIES ARE DEAD — Minh must refresh before Tue Aug 25 23:00 UTC or the sync fails again.** Run 32673142890 (Sun Aug 23): download of Cassie & Colton 1145834202 failed with "The web client only works when logged-in" — the known cookies-expired signature (CLAUDE.md §6 procedure: yt-dlp --cookies-from-browser edge → filter vimeo lines → base64 → update VIMEO_COOKIES_B64 secret). This is a DIFFERENT failure than Sat Aug 22's "no impersonate target" (fixed by Minh's curl-cffi commit a6524d96). Nothing uploaded Sun; 0 uploaded 1 failed.
- **Watch Tuesday's sync run (Aug 25, 23:00 UTC):** first live run with both guards — expect "⛔ Skipping" lines for couples already on the channel, "🚫 Skipping 3" non-wedding, then Cassie & Colton uploading IF cookies were refreshed.
