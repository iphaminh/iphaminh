...
- **⏳ AWAITING "ship it" — 3 commits on `claude/website-work-history-ynl3hj` ahead of main (as of 2026-08-24):** `7bf608a6` YouTube couple-dedup guard (blocks the 14 queued duplicate uploads — TIME-SENSITIVE, sync fires Sun/Tue/Thu/Sat 23:00 UTC), `2a300547` robots.txt newline, `19502cbc` Bing meta fixes (every title ≤65 / description ≤160, verified across all 56 prerendered pages; blogPosts gained a `metaTitle` field; film titles via shared `filmPageTitle()` in filmEnrichment.js). One "ship it" = merge to main, deploys all three.
- **Bing URL Inspection results decoded (2026-08-24):** Bing's data is from a **17 Jul crawl — before the Aug 10 overhaul** — so "page is a redirect" (trailing-slash /napa-valley/), "/cine title too long", and "/cine H1 missing" all describe the OLD site and clear on recrawl. The one still-real issue (descriptions over 160 chars) is fixed in `19502cbc`. After deploy: Minh should URL-Submit the money pages in Bing Webmaster to force recrawl.
- **OPEN question to Minh:** next YouTube sync candidate is "B-You Dentistry Commercial" — block non-wedding videos from the sync, or let them post?
